  ...
  Widget _buildMobileFilterLayout() {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            // Wrapped in Expanded to prevent the "hasSize" layout error
            Expanded(child: _statusDropdown(true)),
  ...
  Widget _buildWebFilterLayout() {
    return Row(
      children: [
        ...
        // Set to false for Web so it only takes needed space
...
// --- REFACTORED STATUS DROPDOWN ---
  Widget _statusDropdown(bool isExpanded) {
    return Container(
...
// Keep your existing _searchField and _resetSearch logic exactly as they are.
        ...
        builder: (context, constraints) {
          ...
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, pSnap) {
              ...
              // --- VIEW SWITCHER ---
              if (isMobile) {
                return _buildMobileListView(filteredList, currentMonthPaidMap, anyWinnerThisMonth, currencyFormat);
              } else {
                return _buildWebTableView(filteredList, currentMonthPaidMap, anyWinnerThisMonth, currencyFormat);
              }
            },
          );
        },
...
// --- NEW MOBILE VIEW ---
  Widget _buildMobileListView(List<dynamic> filteredList, Map<String, dynamic> currentMonthPaidMap, bool anyWinnerThisMonth, NumberFormat currencyFormat) {
    return ListView.separated(
      // IMPORTANT: Attach the same controller used in your Web listener
      controller: _verticalScroll,
      // physics: const AlwaysScrollableScrollPhysics(), // Ensures scroll is active even with few items
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),

      // Add 1 to the length to show the "Loading More" spinner at the bottom
      itemCount: filteredList.length + (_hasMore ? 1 : 0),

      separatorBuilder: (context, index) => const SizedBox(height: 4),
      itemBuilder: (context, index) {

        // 1. Check if we reached the bottom to show the loading indicator
          ...
          return Padding(
            ...
            child: Center(
              child: _hasMore
                  ...
                  : Text("No more members", style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
            ),
          );
        }
        ...
        Color rowBg = hasWonInThisMonth ? Colors.amber.shade50 : (wonInPast ? Colors.green.shade50 : Colors.white);
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Text(d['kuriNumber']?.toString() ?? "-",
                    ...
                    Expanded(
                      ...
                      child: Row(
                        ...
                        children: [
                          _compactInfoCol("PAID", currencyFormat.format(totalPaidSoFar), Colors.green.shade700),
                          const SizedBox(width: 12),
                          _compactInfoCol("BAL", currencyFormat.format(balance < 0 ? 0 : balance), balance > 0 ? Colors.red.shade700 : Colors.green.shade700),
                        ],
                      ),
                    ),
                    const SizedBox(width: 8),
                    _buildStatusBadge(hasPaymentDoc, pMonth, winnerMonth),
                  ],
                ),
              ),
              Container(
                ...
                child: Row(
                  children: [
                    Text("INST: $displayPaidCount/$displayTotalCount",
                        style: const TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                    const Spacer(),
                    if (wonInPast)
                      const Text("PAST WINNER", style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, color: Colors.green))
                    else ...[
                      _actionBtn(hasPaymentDoc ? Icons.edit : Icons.add, hasPaymentDoc ? "EDIT" : "PAY", hasPaymentDoc ? Colors.orange : Colors.green,
                              () => _showMarkPaymentDialog(mid, d['name'], d['phone'], d['kuriNumber']?.toString() ?? "-", d['masterId'] ?? mid, monthlyAmount, existingPayment: pMonth)),
                      _actionBtn(Icons.chat_bubble_outline, "WA", Colors.teal,
                              () => _sendWhatsAppReminder(d['phone'] ?? "", d['name'] ?? "Member", balance)),
                      if (hasWonInThisMonth)
                        const Icon(Icons.stars, color: Colors.orange, size: 16)
                      else if (!anyWinnerThisMonth)
                        _actionBtn(Icons.emoji_events_outlined, "WIN", isFullyPaid ? Colors.blue : Colors.grey.shade300,
                            isFullyPaid ? () => _markAsWinner(mid, d['name'], true, pMonth) : null),
                    ],
                    if (widget.userRole == 'Super Admin' && !wonInPast) ...[
                      const VerticalDivider(width: 10, indent: 8, endIndent: 8),
                      _adminAction(Icons.settings, Colors.blueGrey, () => _showEditEnrollmentDialog(currentMemberDoc)),
                      _adminAction(Icons.delete_outline, Colors.red, () => _confirmDeleteEnrollment(m.id, d)),
                    ]
                  ],
                ),
              ),
...
// Sub-helper for Compact Data
  Widget _compactInfoCol(String label, String value, Color color) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.end,
      children: [
        Text(label, style: TextStyle(fontSize: 8, color: Colors.grey.shade500, fontWeight: FontWeight.w900)),
        Text(value, style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: color)),
      ],
    );
  }

// Sub-helper for Compact Buttons
...
// Sub-helper for Admin Icons
  Widget _adminAction(IconData icon, Color color, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      child: Padding(
...
// --- WEB VIEW (YOUR ORIGINAL CODE) ---
  Widget _buildWebTableView(List<dynamic> filteredList, Map<String, dynamic> currentMonthPaidMap, bool anyWinnerThisMonth, NumberFormat currencyFormat) {
...
// Ensure you have a matching _showLoadingDialog or use this one
  ...
  Widget _buildPayButton(String enrollmentId, String name, String phone,String kuriNumber, String masterId, double monthlyAmount) {
    return ElevatedButton(
      ...
      onPressed: () => _showMarkPaymentDialog(enrollmentId, name,phone,kuriNumber, masterId, monthlyAmount),
  ...
  //   );
  // }
  void _showMarkPaymentDialog(String enrollmentId, String name,String phone, String kuriNumber, String masterId, double monthlyAmount, {Map<String, dynamic>? existingPayment}) async {
    // 1. Fetch admins - Keep your existing logic
    final adminSnap = await FirebaseFirestore.instance.collection('staff_admins').orderBy('name').get();
      ...
      builder: (c) => MarkPaymentDialog(
        ...
        onDelete: existingPayment != null && existingPayment.containsKey('id')
          ...
          try {
            ...
            // Make sure this collection name matches your Member/Enrollment collection
            DocumentReference memberRef = FirebaseFirestore.instance.collection('enrollments').doc(enrollmentId);

            double amountToRemove = _parseNum(existingPayment['collectedTotal']);

            batch.delete(payRef);
            // Update the member doc so the table balance stays correct
            batch.update(memberRef, {
              'totalPaid': FieldValue.increment(-amountToRemove),
  ...
  Future<void> migrateMemberTotals() async {
    try {
      debugPrint("🚀 Starting Migration: Fetching data...");

      // 1. Fetch all enrollments
      final enrollmentSnap = await FirebaseFirestore.instance.collection('enrollments').get();
      debugPrint("📂 Found ${enrollmentSnap.docs.length} members in enrollments.");

      // 2. Fetch all payments
      final paymentSnap = await FirebaseFirestore.instance.collection('payments').get();
      debugPrint("💰 Found ${paymentSnap.docs.length} total payment records.");

      WriteBatch batch = FirebaseFirestore.instance.batch();
      int count = 0;
      int totalProcessed = 0;

      for (var memberDoc in enrollmentSnap.docs) {
        String mid = memberDoc.id;
        final mData = memberDoc.data();
        String mName = mData['name'] ?? "Unknown";

        // Find all payments belonging to this member
        final memberPayments = paymentSnap.docs.where((p) => p['memberId'] == mid).toList();

        double total = 0;
        int monthsPaid = memberPayments.length;

        for (var p in memberPayments) {
          final pData = p.data();
          total += (pData['collectedTotal'] ?? pData['amount'] ?? 0).toDouble();
        }

        // Update the enrollment document with SetOptions(merge: true)
        batch.set(memberDoc.reference, {
          'totalPaid': total,
          'paidMonthsCount': monthsPaid,
        }, SetOptions(merge: true));

        debugPrint("✅ Prepared: $mName ($mid) -> Total: ₹$total, Months: $monthsPaid");
        ...
        if (count >= 450) {
          debugPrint("⏳ committing batch of 450 records...");
          await batch.commit();
      ...
      if (count > 0) {
        await batch.commit();
        debugPrint("📥 Final batch committed.");
      }

      debugPrint("🏁 Migration Finished! Successfully processed $totalProcessed members.");
  ...
  Future<void> migratePaymentDetails() async {
    try {
      debugPrint("🚀 Starting Payment Detail Migration...");

      // 1. Fetch all enrollments
      final enrollmentSnap = await FirebaseFirestore.instance.collection('enrollments').get();

      // Quick lookup map
      Map<String, Map<String, dynamic>> memberLookup = {
        for (var doc in enrollmentSnap.docs) doc.id: doc.data()
      };

      // 2. Fetch all payments
      final paymentSnap = await FirebaseFirestore.instance.collection('payments').get();
      debugPrint("📂 Found ${paymentSnap.docs.length} payments to check.");
      ...
      for (var payDoc in paymentSnap.docs) {
        ...
        if (mid != null && memberLookup.containsKey(mid)) {
          final mInfo = memberLookup[mid]!;

          // Use batch.update (No SetOptions needed here)
          batch.update(payDoc.reference, {
            'memberName': mInfo['name'] ?? "Unknown",
            'kuriNumber': mInfo['kuriNumber'] ?? "-",
            'phone': mInfo['phone'] ?? "-",
          });
          ...
          if (count >= 450) {
            debugPrint("⏳ Committing batch of 450 payments...");
            await batch.commit();
            batch = FirebaseFirestore.instance.batch();
            count = 0;
          }
      ...
      debugPrint("🏁 Done! Updated $updatedCount payment records.");
    } catch (e) {
      debugPrint("❌ Migration Error: $e");
    }
